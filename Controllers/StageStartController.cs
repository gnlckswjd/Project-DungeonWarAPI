...
public class StageStartController : ControllerBase
{
	...
	private async Task<ErrorCode> CheckStageAccessibilityAsync(Int32 gameUserId, Int32 selectedStageLevel,
		UserStateCode state)
	{
		...
		List<StageItem> itemList, List<StageNpc> npcList, Int32 selectedStageLevel)
	{
		...
		var errorCode = await _memoryDatabase.StoreStageDataAsync(stageKey, stageKeyValueList);
		if (errorCode != ErrorCode.None)
		{
			return errorCode;
		}
